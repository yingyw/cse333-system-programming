...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  // MISSING:
  HWSize_t wh = WriteHeader(f, dtres, mires);
  if (wh == 0) {
    fclose(f);
    unlink(filename);
    return 0;
...
static HWSize_t WriteDocidDocnameFn(FILE *f,
  ...
  res = fwrite(filename, 1, slen_ho, f);
  if (res != slen_ho) {
    return 0;
  }
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    // MISSING:
    LLIteratorGetPayload(it, (void **) &position);
...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  if (fwrite(wds->word, 1, wordlen_ho, f) != wordlen_ho){
    return 0;
  }

  // Calculate and return the total amount of data written.
  // MISSING (fix this return value):
  return wordlen_ho + htlen_ho + sizeof(header);
}
...
static HWSize_t WriteHeader(FILE *f,
  ...
  for (HWSize_t i = 0; i < cslen; i++) {
    fread(&buffer, 1, 1, f);
    crcobj.FoldByteIntoCRC((uint8_t)buffer);
  }
...
static HWSize_t WriteBucketRecord(FILE *f,
  ...
  bucket_rec br;
  // MISSING:
  uint32_t chainLen = NumElementsInLinkedList(li);
  br = {chainLen, b_offset};
  br.toDiskFormat();
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // MISSING:
      element_position_rec prec = {nextelpos};
      prec.toDiskFormat();
      res = fseek(f, offset, SEEK_SET);
      if (res != 0) {
        return 0;
      }
      res = fwrite(&prec, sizeof(prec), 1, f);
      if (res != 1) {
        return 0;
      }
      offset += sizeof(prec);
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    ...
    if (NumElementsInLinkedList(bucket) == 0) {
      res = WriteBucketRecord(f, bucket, next_bucket_rec_offset, next_bucket_offset);
    } else {
      res = WriteBucketRecord(f, bucket, next_bucket_rec_offset, next_bucket_offset);
      size = WriteBucket(f, bucket, next_bucket_offset, fn);
    }
    next_bucket_rec_offset += sizeof(bucket_rec);
    next_bucket_offset += size;
  }
