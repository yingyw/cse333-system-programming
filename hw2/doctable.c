...
DocID_t DTRegisterDocumentName(DocTable table, char *docname) {
  ...
  // STEP 2.
  if (LookupHashTable(table->docname_to_docid,
      FNVHash64((unsigned char *)docname, strlen(docname)),
      &oldkv) == 1) {
      free(doccopy);
      free(docid);
      return *((char*)(oldkv.value));
  }
...
char *DTLookupDocID(DocTable table, DocID_t docid) {
  ...
  if (LookupHashTable(table->docid_to_docname,
      docid, &kv) == 0 || LookupHashTable(table->docid_to_docname,
      docid, &kv) == -1) {
      return NULL;
  }
  return kv.value;
