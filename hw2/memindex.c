...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    wds->word = word;
    HashTable table = AllocateHashTable(5);
    wds->docIDs = table;
    kv.value = wds;
    kv.key = wordkey;
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  res = LookupHashTable(index, FNVHash64((unsigned char*)query[0],
    strlen(query[0])), &kv);
  ...
  if (res != 1) {
    ...
    wds = kv.value;
    HTIter itr = HashTableMakeIterator(wds->docIDs);
    // check if iterator has go over all elements
    while (!HTIteratorPastEnd(itr)) {
      SearchResult *search = (SearchResult *)malloc(sizeof(SearchResult));
      res = HTIteratorGet(itr, &kv);
      search->docid = kv.key;
      search->rank = NumElementsInLinkedList(kv.value);
      PushLinkedList(retlist, (void *)search);
      HTIteratorNext(itr);
    }
    HTIteratorFree(itr);
  }
  ...
  for (i = 1; i < qlen; i++) {
    ...
    res = LookupHashTable(index, FNVHash64((unsigned char*)query[i],
      strlen(query[i])), &kv);
    ...
    // check for all element in linked list
    for (j = 0; j < ne; j++) {
       SearchResult *search;
       LLIteratorGetPayload(llit, (LLPayload_t*)&search);
       if (LookupHashTable(((WordDocSet*)(kv.value))->docIDs,
        search->docid, &htk) == 1) {
            // update rank if there is match
            search->rank += NumElementsInLinkedList(htk.value);
            LLIteratorNext(llit);
       } else {
            // delete the docID from search result list if no match
